...
let state = null;
let ui = { mode: 'idle', modal: null, rolling: false };
let modalEl = null;
...
function newGame() { clearSave(); state = null; ui = { mode: 'idle', modal: null, rolling: false }; removeModal(); renderSetup(); }
  ...
  const setup = h('div', { class: 'setup' }, [
    ...
    saved ? h('div', { class: 'card' }, [
      h('div', { class: 'toggle-row' }, [
        ...
        h('button', { class: 'btn btn-primary btn-sm', text: '▶ Resume', on: { click: () => { state = saved; ui = { mode: 'idle', modal: null, rolling: false }; applyTheme(saved.config.theme); render(); } } }),
      ]),
    ]) : null,
    h('div', { class: 'card' }, [
      h('h2', { text: 'Players' }),
...
function startGame() {
  ...
  ui = { mode: 'idle', modal: null, rolling: false };
  applyTheme(cfg.theme);
  save(state);
  render();
}
...
function demoBoot(params) {
  ...
  state = s; ui = { mode: 'idle', modal: null, rolling: false };
